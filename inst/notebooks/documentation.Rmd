...
```{r createattrvarnum}
# We can also set the nunber of attributes to a value other than one
tiledb:::libtiledb_attribute_set_cell_val_num(attr@ptr, 3)

# And we can set the a variable nunber of attributes (using NA to select variable size)
...
```{r schemaload}
array_name <- "/tmp/tiledb/quickstart_sparse"
A <- tiledb_sparse(uri = array_name)
# can also load dense
sch <- schema(A)
