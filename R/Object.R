check_object_arguments <- function(ctx, uri) {
...
#' 
#'@export
tiledb_group_create <- function(ctx, uri) {
...
#' 
#' @export
tiledb_object_type <- function(ctx, uri) {
  check_object_arguments(ctx, uri)
...
#' @export
tiledb_object_rm <- function(ctx, uri) {
  check_object_arguments(ctx, uri)
...
#' @export
tiledb_object_mv <- function(ctx, old_uri, new_uri) {
  if (missing(ctx) || !is(ctx, "tiledb_ctx")) {
    stop("argument ctx must a a tiledb_ctx")
  }
...
#' @export
tiledb_object_ls <- function(ctx, uri, filter = NULL) {
  check_object_arguments(ctx, uri) 
...
#' @return a dataframe with object type, object uri string columns
#' @export
tiledb_object_walk <- function(ctx, uri, order = "PREORDER") {
  check_object_arguments(ctx, uri)
