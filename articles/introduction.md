...
documentation](https://tiledb-inc.github.io/TileDB-R/articles/documentation.md)
is also available.

## Getting started

Once the TileDB R package is installed, it can be loaded via
[`library(tiledb)`](https://github.com/TileDB-Inc/TileDB-R).
Installation is supported for Windows, Linux and macOS via the [official
CRAN package](https://cran.r-project.org/package=tiledb), on Linux and
macOS via the [conda](https://anaconda.org/conda-forge) package as well
as from source.

Documentation for the TileDB R package is available via the
[`help()`](https://rdrr.io/r/utils/help.html) function from within R as
well as via the [package
...
`s3://some/data/bucket` can be used where a local file would be used.
See the script
[ex_S3.R](https://github.com/TileDB-Inc/TileDB-R/blob/main/inst/examples/ex_S3.R)
for an example.
...
These illustrations use the array created by the file
[ex_1.R](https://github.com/TileDB-Inc/TileDB-R/blob/main/inst/examples/ex_1.R)
...
The file
[ex_1.R](https://github.com/TileDB-Inc/TileDB-R/blob/main/inst/examples/ex_1.R)
in the examples directory is a simple yet complete example extending
[quickstart_dense.R](https://github.com/TileDB-Inc/TileDB-R/blob/main/inst/examples/quickstart_dense.R)
...
to access the array; the older variants
...
remain supported but are deprecated and may be removed at some point in
the future.

*Read 1-D*

The first example extracts rows 1 to 2 and column 2 from an array. It
also limits the selection to just one attribute (via `attrs`), asks for
the return to be a `data.frame` (instead of a simpler list) and for the
(row and column, if present as here) indices to not be printed (via
`extended=FALSE`).
...
attributes can be selected via the `attrs` argument, or the
[`attrs()`](https://tiledb-inc.github.io/TileDB-R/reference/generics.md)
method on the array object. The square-bracket indexing then selects
with in the 2-d attribute object.

If multiple objects are returned (as `list` or `data.frame`), subsetting
on the returned object works via `[[var]]` or `$var`. A numeric index
also works (but needs to account for `rows` and `cols`).
...
This works analogously. Note that the results are generally returned as
vectors, or as a columns of a `data.frame` object in case that option
was set.
...
We can restrict the selection to a subset of attributes when opening the
array.
...
as a factor variable as is still the R 3.6.\* default which is about to
change. We can also override, users can too.)
...
Consistent with the `data.frame` semantics, *now* requesting a named
column *reduces to a vector* as this happens at the R side:
...
We can also request a `data.frame` object, either when opening or by
changing this object characteristic on the fly:
...
For sparse arrays, the return type is by default ‘extended’ showing rows
and column but this can be overridden.
...
[ex_2.R](https://github.com/TileDB-Inc/TileDB-R/blob/main/inst/examples/ex_2.R)
...
The row coordinate is currently a floating point representation of the
underlying time type. We can both select attributes (here we excluded
the “a” column) and select rows by time (as the time stamps get
...
More extended examples are available showing indexing by date(time) as
well as character dimension.

## Additional Information

The TileDB R package is documented via R help functions (*e.g.*
[`help("tiledb_array")`](https://tiledb-inc.github.io/TileDB-R/reference/tiledb_array.md)
...
TileDB itself has extensive
[installation](https://docs.tiledb.com/developer/installation), and
[overall documentation](https://docs.tiledb.com/developer/) as well as a
[support forum](https://forum.tiledb.com/).
