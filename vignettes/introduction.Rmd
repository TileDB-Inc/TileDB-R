...
supported for Windows, Linux and macOS via the [official CRAN package][cran_tileb], on Linux and
macOS via [the conda package](https://anaconda.org/conda-forge/r-tiledb) as well as from source.
...
`rows` and `cols`).
    ...
    R> A <- tiledb_array(uri = uri, attrs = c("a","b"), as.data.frame = TRUE)
    R> A[1:2,2][["a"]]
    [1] 2 7
    R> A[1:2,2]$a
    [1] 2 7
...
We can restrict the selection to a subset of attributes when opening the array.
    ...
    R> A <- tiledb_dense(uri = uri, attrs = c("b","c"), as.data.frame = TRUE, extended=FALSE)
    R> A[6:9,2:4]
           b     c
    1  114.0 brown
    2  114.5   fox
...
which is about to change. We can also override, users can too.)
    ...
    R> A <- tiledb_array("/tmp/tiledb/ex_1/", attrs=c("b","c"), as.data.frame = TRUE, extended=TRUE)
...
created by the [quickstart_sparse][qs_sparse] example.
    ...
    R> A <- tiledb_array(uri = uri, is.sparse = TRUE)
    R> A[]
    $rows
    [1] 1 2 2
...
just integer and double precision floats.
    ...
    R> A <- tiledb_array(uri = uri, as.data.frame = TRUE)
    R> A[1577858403:1577858408]
            rows cols a   b          d                       e
...
get converted to the required floating point value).
    ...
    R> attrs(A) <- c("b", "d", "e")
    R> A[as.POSIXct("2020-01-01 00:00:00"):as.POSIXct("2020-01-01 00:00:03")]
            rows cols   b          d                       e
...
documentation][tiledb-r_docs].  An extended [notebook][tiledb_notebook] is available, as is a
...
[tiledb_notebook]: https://tiledb-inc.github.io/TileDB-R/documentation.nb.html
